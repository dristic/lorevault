...
pub async fn run(server: Option<String>) -> anyhow::Result<()> {
    let server = match server {
        ...
        None => CliConfig::load()?
            .server
            .ok_or_else(|| anyhow::anyhow!("no server specified — pass one or run `lorevault login` first"))?,
    };

    let client = ApiClient::new(server.clone(), None);
    let (status, body) = client.get_public_raw("/healthz").await?;

    let reported_status = body.get("status").and_then(Value::as_str).unwrap_or("unknown");
